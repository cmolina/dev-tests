...
import { Variety } from '../variety/variety.entity.js'
...
beforeAll(async () => {
  ...
  fruit = em.create(Fruit, { name: 'brocoli' })
  variety = em.create(Variety, { name: 'large' })
  ...
  await em.persistAndFlush([client, fruit, variety, field])
})
...
it('should add a new crop', async () => {
  const body = JSON.stringify({ client: client.id, fruit: fruit.id, variety: variety.id, field: field.id } as NewCropPayload)

  const res = await app.request('/crops', { method: 'post', body })

  expect(res.status).toBe(201)
  expect(await res.json()).toEqual(expect.objectContaining({ client: client.id, fruit: fruit.id, variety: variety.id, field: field.id }))
})
...
it('should persist new crops', async () => {
  ...
  expect(res.status).toBe(200)
  expect(await res.json()).toContainEqual(expect.objectContaining({ client, fruit, variety, field }))
})
